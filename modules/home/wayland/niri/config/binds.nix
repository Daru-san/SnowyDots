          ...
          let
            ...
            mkBinds =
              ...
              map (
                ...
                (
                  let
                    key_str = builtins.toString key;
                    fkey_str = if key == 0 then "F10" else "F${key_str}";
                    ws = if key == 0 then 10 else key;
                    fws = if key == 0 then 20 else key + 10;
                  in
                  {
                    "Mod+${key_str}".action.focus-workspace = ws;
                    "Mod+Ctrl+${key_str}".action.move-column-to-workspace = ws;
                    "Mod+Shift+${key_str}".action.move-window-to-workspace = ws;
                    "Mod+${fkey_str}".action.focus-workspace = fws;
                    "Mod+Ctrl+${fkey_str}".action.move-column-to-workspace = fws;
                    "Mod+Shift+${fkey_str}".action.move-window-to-workspace = fws;
                  }
                )
              ) keys;
          in
          lib.mergeAttrsList (mkBinds keys)
        )
        (
          let
