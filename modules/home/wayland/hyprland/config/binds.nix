...
in {
  ...
  in {
    ...
    in [
      (mkBindE "super" "e"
        "hyprctl clients | grep 'nautilus' || ${file-manager}"
        "Launch file manager")
      ...
      (mkBindE "super" "z" "${terminal} --detach ${editor}"
        "Launch a text editor")
      (mkBindE "super" "m" "${terminal} --detach ${btop}"
        "Launch a system monitor")
      ...
      (mkBindE "super" "F4" "${obs} recording toggle"
    ...
    in [
      ...
      (mkBindE "super" "i"
        "${pk} nmtui || ${kitty} --class nmtui --detach nmtui"
      ...
      # Audio mixer
      (mkBindE "super" "p"
        "${pk} pulsemixer || ${kitty} --class pulsemixer --detach pulsemixer"
        "Launch pulsemixer")

      # Screenshotting
      (mkBindSE "print"
        "${pk} hyprshot || ${hyprshot} -m region -z -o ~/Pictures/Screenshots"
        "Take a screenshot of a selected region")
      (mkBindE "alt" "print"
        "${pk} hyprshot || ${hyprshot} -m active -z -o ~/Pictures/Screenshots"
