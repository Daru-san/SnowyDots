...
export class Device extends Gio.DBusProxy {
  static {
    ...
    this[GObject.properties] = {
      ...
      state: GObject.ParamSpec.uint(
        ...
        GObject.ParamFlags.READABLE,
        DeviceState.NONE,
        DeviceState.CONNECTED | DeviceState.PAIR_OUTGOING,
        DeviceState.NONE,
      ),
    }
  ...
  on_g_properties_changed(changed, _invalidated) {
    try {
      ...
      for (const name in changed.deepUnpack()) this.notify(properties[name])
    } catch (e) {
      console.error(e)
    }
  }
  ...
  get(name, fallback = null) {
    ...
    if (value instanceof GLib.Variant) return value.unpack()
 ...
 */
export class ValService extends Gio.DBusProxy {
  static {
  ...
  on_g_signal(senderName_, signalName, parameters) {
    ...
    if (signalName === 'InterfacesAdded') this.#onInterfacesAdded(...args)
  ...
  *[Symbol.iterator]() {
  ...
  #initCallback(service, result) {
    try {
      ...
      this.#nameOwnerChangedId = this.connect(
        'notify::g-name-owner',
  ...
  #deviceInitCallback(device, result) {
    try {
      ...
      if (this.#items.some((item) => item.id === device.id)) return
  ...
  activate_action(name, target = null) {
    ...
    if (target instanceof GLib.Variant) parameters[0] = target
